...
import random
import config
from utils import convert_debug_data_to_final_reviews, load_asin_info_from_excel, save_failed_asin, save_results_to_json, save_debug_results_to_json
...
import os
from db_saver import MongoSaver
...
def parse_arguments():
    ...
    parser.add_argument('--login_headless', action='store_true', help='로그인 단계(Selenium 기반)에서 headless 모드 사용')
    ...
    parser.add_argument('--crawling_headless', action='store_true', help='리뷰 크롤링 단계(Playwright 기반)에서 headless 모드 사용')
...
def main():
    ...
    logger.info(f"로그인 브라우저: {args.login_browser}, Login Headless: {args.login_headless}")
    ...
    if args.use_proxy:
        if PROXIES:
            ...
            logger.warning("프록시 목록이 비어 있습니다. 프록시 없이 진행합니다.")
    else:
        logger.info("프록시 미사용(내 IP로 접속)")
    ...
    try:
        ...
        logger.info(f"VPN/프록시 정보 확인(ipify.org): {ip_info}")
        ...
        logger.info(f"ASIN 슬라이싱 범위: {asin_start} ~ {asin_end}, 실제 로드된 ASIN 개수: {len(asin_info)}")
        ...
        all_results, failed_info = asyncio.run(crawl_reviews_for_asin_list_playwright(
            ...
            headless=args.crawling_headless, 
            max_concurrent_contexts=args.max_concurrent_contexts,
            logger=logger
        ))
        timer.stop()

        # 크롤링 결과 저장
        # 1. DEBUG 용 전체 구조 저장 (asin 구분 없이 전체 리뷰에 순차적으로 seq 부여)
        seq_counter = 1
        for asin_dict in all_results:
            for review in asin_dict.get("result", []):
                review["seq"] = seq_counter
                seq_counter += 1
        debug_output_path = reviews_output_path.replace(".json", "_DEBUG.json")
        save_debug_results_to_json(all_results, debug_output_path, logger=logger)

        # 2. 리뷰만 평탄화(flatten)해서 저장
        flat_reviews = convert_debug_data_to_final_reviews(all_results)

        if flat_reviews:
            # 최종 JSON에는 seq를 부여하지 않고 저장
            save_results_to_json(flat_reviews, reviews_output_path, logger=logger)
        else:
            logger.info("저장할 리뷰가 없습니다. (모든 ASIN이 리뷰 없음)")

        if failed_info:
            save_failed_asin(failed_info, failed_output_path, logger=logger)

        # MongoDB 적재: seq는 db_saver.py에서 auto increment로 부여
        try:
            db_saver = MongoSaver()
            final_output_path = reviews_output_path.replace("_DEBUG", "")
            # flat_reviews에는 seq가 없음. db_saver.save_json_file에서 seq를 부여하며 insert
            db_saver.save_json_file(final_output_path)
        except Exception as e:
            logger.error(f"MongoDB 적재 실패: {e}")
