...
def save_results_to_json(results, filename, logger=None):
    ...
    logger.info(f"데이터를 저장했습니다: {filename}")

def save_debug_results_to_json(results, filename, logger=None):
    if logger is None:
        logger = logging.getLogger()
    logger.info(f"DEBUG 결과 저장 시도")
    dir_path = os.path.dirname(filename)
    if dir_path and not os.path.exists(dir_path):
        os.makedirs(dir_path, exist_ok=True)
    with open(filename, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    logger.info(f"DEBUG 데이터 저장 완료: {filename}")
...
def convert_debug_data_to_final_reviews(debug_results):
    """
    메타데이터가 포함된 debug용 데이터(ASIN별 dict + result)에서 리뷰만 리스트로 평탄화하여 반환
    """
    reviews = []
    for asin_dict in debug_results:
        reviews.extend(asin_dict.get("result", []))
    return reviews