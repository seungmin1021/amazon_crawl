...
from extractors_playwright import (
    ...
    extract_review_count, extract_group_id
)
...
async def crawl_reviews_for_asin_context(browser, selenium_cookies, asin, meta_info, max_pages=MAX_PAGES, logger=None):
    if logger is None:
    ...
    try:
        while page_num <= max_pages:
            if page_num == 1:
                ...
                logger.info(f"[크롤링] ASIN {asin} - 총 리뷰 개수: {review_cnt}")
            ...
            try:
                ...
                if page_num == 1:
                    logger.warning(f"[크롤링] ASIN {asin} - 리뷰 페이지에서 리뷰 블록을 찾지 못함. 제품 존재 여부 확인 시작.")
                    ...
                    logger.info(f"[크롤링] ASIN {asin} - 페이지 {page_num}에서 더 이상 리뷰 블록을 찾지 못해 크롤링을 종료합니다.")
                    break
            ...
            for review in review_blocks:
                try:
                    ...
                    write_dt_raw = await extract_first_text_by_selectors(review, REVIEW_DATE_SELECTORS, logger=logger)
                    write_dt = ""
                    if write_dt_raw:
                        import re
                        ...
                        if match:
                            try:
                                from datetime import datetime
                                write_dt = datetime.strptime(match.group(1), "%B %d, %Y").strftime("%Y-%m-%d")
                            except Exception:
                                write_dt = ""
                    ...
                    logger.error(f"[크롤링] ASIN {asin} 리뷰 데이터 추출 중 오류: {e}")
...
async def crawl_reviews_for_asin_list_playwright(
    selenium_cookies, asin_list, reviews_output_path, failed_output_path, 
    ...
    async with async_playwright() as p:
        ...
        async def crawl_one(meta):
            ...
            async with sem:
                reviews, fail_reason, failure_type = await crawl_reviews_for_asin_context(browser, selenium_cookies, asin, meta, max_pages, logger=logger)
        ...
        results_with_failures = await asyncio.gather(*tasks)

        for result, failure_info in results_with_failures:
            if result:
                all_results.append(result)
            if failure_info:
                all_failed_info.append(failure_info)

        save_results_to_json(all_results, filename=reviews_output_path, logger=logger)
        logger.info(f"[최종 요청수] 총 {len(all_results)}개 ASIN에 대한 결과 저장 완료")
        ...
        if all_failed_info:
            ...
            with open(output_path, "w", encoding="utf-8") as f:
                json.dump(all_failed_info, f, ensure_ascii=False, indent=2)
            logger.info(f"[저장][Failed] 실패 ASIN 정보 {len(all_failed_info)}건 저장 완료: {output_path}")

        await browser.close()
    return all_results, all_failed_info 