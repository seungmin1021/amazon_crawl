    ...
    for sel_type, sel_value in selectors:
        try:
            if sel_type == "css":
                ...
                continue
            if blocks:
                logger.debug(f"리뷰 블록 추출 성공: {len(blocks)}개")
                return blocks
        except Exception as e:
            logger.error(f"리뷰 블록 추출 오류 - {e}, selector={sel_type}:{sel_value}")
            continue
    logger.warning(f"리뷰 블록 추출 실패")
    return []
...
async def strip_html_tags(text, logger=None):
    ...
    try:
        if not text:
            return ""
        clean = re.sub('<.*?>', '', text)
        logger.debug(f"HTML 태그 제거 결과: {clean}")
        return clean.strip()
    except Exception as e:
        logger.error(f"HTML 태그 제거 오류 - {e}")
        return ""
    ...
    try:
        for sel in selectors:
            ...
            for elem in elems:
                ...
                if val:
                    logger.debug(f"셀렉터 추출 성공: {val}")
                    return str(val)
        logger.debug("셀렉터 추출 결과 없음 (type: str)")
        return ""
    except Exception as e:
        logger.error(f"셀렉터 추출 오류 - {e}, selectors={selectors}")
        return ""
    ...
    try:
        for sel_type, sel_value in selectors:
            ...
            for elem in elems:
                ...
                if val:
                    logger.debug(f"속성 추출 성공: {val}")
                    return val.strip()
        logger.debug(f"속성 추출 실패")
        return ""
    except Exception as e:
        logger.error(f"속성 추출 오류 - {e}, selectors={selectors}, attr={attr}")
        return ""
        ...
        for sel_type, sel_value in REVIEW_TITLE_SELECTORS:
            ...
            for elem in elems:
                ...
                if text and text.strip():
                    logger.debug(f"리뷰 제목 추출 성공: {text}")
        ...
        if a_title:
            ...
            for span in spans:
                ...
                if not class_attr or "cr-original-review-content" in class_attr:
                    ...
                    if text and text.strip():
                        logger.debug(f"리뷰 제목 fallback 추출 성공: {text}")
                        return str(text).strip()
        logger.debug("리뷰 제목 추출 결과 없음 (type: str)")
        return ""
    except Exception as e:
        logger.error(f"리뷰 제목 추출 오류 - {e}, selectors={REVIEW_TITLE_SELECTORS}")
        return ""
...
async def extract_review_content(review, REVIEW_CONTENT_SELECTORS, logger=None):
    ...
    logger.debug(f"리뷰 본문 추출 실행, Selectors: {REVIEW_CONTENT_SELECTORS}")
    try:
        ...
        for sel_type, sel_value in REVIEW_CONTENT_SELECTORS:
            ...
            if review_body_el:
                logger.debug(f"리뷰 본문 컨테이너 찾음 (selector: {sel_type}='{sel_value}')")
                ...
                if content and content.strip():
                    ...
                    logger.debug(f"리뷰 본문 추출 성공: {clean_content}")
                    return clean_content
                
        logger.warning("모든 selector로 리뷰 본문을 추출하지 못했습니다.")
        return ""
    except Exception as e:
        logger.error(f"리뷰 본문 추출 오류 - {e}")
        return ""
    ...
    try:
        ...
        if url:
            ...
            if url.startswith("http"):
                logger.debug(f"절대 URL 반환: {url}")
                return url
            else:
                logger.debug(f"상대 URL 반환: {base_url + url}")
                return base_url + url
        logger.debug(f"링크 없음")
        return ""
    except Exception as e:
        logger.error(f"오류 발생 - {e}, selectors={REVIEW_TITLE_LINK_SELECTORS}")
        return ""
    ...
    try:
        for sel_type, sel_value in REVIEW_WRITER_SELECTORS:
            ...
            for elem in elems:
                val = await elem.inner_text()
                if val:
                    logger.debug(f"추출 성공: {val}")
                    return str(val).strip()
        logger.debug("추출 결과 없음 (type: str)")
        return ""
    except Exception as e:
        logger.error(f"오류 발생 - {e}, selectors={REVIEW_WRITER_SELECTORS}")
        return ""

async def extract_write_dt(review, selectors, logger=None):
    if logger is None:
        logger = logging.getLogger()
    try:
        write_dt_raw = await extract_first_text_by_selectors(review, selectors, logger=logger)
        if write_dt_raw:
            import re
            from datetime import datetime
            
            match = re.search(r"on ([A-Za-z]+ \d{1,2}, \d{4})", write_dt_raw)
            if match:
                try:
                    dt_obj = datetime.strptime(match.group(1), "%B %d, %Y")
                    formatted_dt = dt_obj.strftime("%Y-%m-%d")
                    logger.debug(f"날짜 추출 및 변환 성공: {formatted_dt}")
                    return formatted_dt
                except ValueError as ve:
                    logger.warning(f"날짜 형식 변환 오류: {ve}, 원본: {match.group(1)}")
                    return ""
            else:
                logger.debug(f"원본 텍스트에서 날짜 패턴을 찾지 못함: {write_dt_raw}")
                return ""
        else:
            logger.debug("날짜 정보가 포함된 원본 텍스트를 찾지 못함")
            return ""
    except Exception as e:
        logger.error(f"오류 발생 - {e}, selectors={selectors}")
        return ""
...
async def extract_option(review, REVIEW_OPTION_SELECTORS, REVIEW_OPTION_EXCLUDE_KEYWORDS, logger=None):
    ...
    logger.debug(f"실행, Selectors: {REVIEW_OPTION_SELECTORS}")
    try:
        all_option_texts = []
        ...
        for selector in REVIEW_OPTION_SELECTORS:
            try:
                ...
                logger.warning(f"내부 옵션 추출 오류: {e}, selector={selector}")
                continue

        if not all_option_texts:
            logger.debug("추출된 옵션 없음")
            return ""
        ...
        logger.debug(f"추출 성공: {clean_final_options}")
...
async def extract_is_verified(review, REVIEW_VERIFIED_SELECTORS, logger=None):
    ...
    try:
        for selector in REVIEW_VERIFIED_SELECTORS:
            ...
            if elems:
                logger.debug(f"추출 성공: True (type: bool)")
                return True
        logger.debug(f"추출 실패: False (type: bool)")
        return False
    except Exception as e:
        logger.error(f"오류 발생 - {e}, selectors={REVIEW_VERIFIED_SELECTORS}")
        return False
            ...
            for elem in elems:
                ...
                if text:
                    ...
                    if "No customer reviews" in text:
                        logger.debug(f"'No customer reviews' found.")
                        return 0
                    ...
                    if match:
                        ...
                        logger.debug(f"Extracted count: {count_str}")
                        return int(count_str)
        except Exception as e:
            logger.error(f"Error with selector {sel_type}:{sel_value} - {e}")
            continue
...
def parse_helpful(text, logger=None):
    ...
    try:
        ...
        if m:
            logger.debug(f"추출 성공: {m.group(1)}")
            return int(m.group(1))
        logger.debug("추출 실패")
        return 0
    except Exception as e:
        logger.error(f"오류 발생 - {e}, text={text}")
        return 0
        ...
        for sel_type, sel_value in REVIEW_STAR_SELECTORS:
            ...
            for elem in elems:
                ...
                if rating_text:
                    ...
                    logger.debug(f"selector={sel_type}:{sel_value}, text={rating_text}, star={star}")
                    if star > 0:
                        return int(round(star)) if as_int else star
        logger.warning("평점 추출 실패, 0 반환")
        return 0 if as_int else 0.0
    except Exception as e:
        logger.error(f"오류 발생 - {e}, selectors={REVIEW_STAR_SELECTORS}")
        return 0 if as_int else 0.0
                ...
                for link in option_links:
                    ...
                    if href:
                        ...
                        if match:
                            ...
                            logger.debug(f"옵션 href에서 group_id 추출 성공: {group_id}")
                            return group_id
            except Exception as e:
                logger.warning(f"selector 처리 중 오류: {e}, selector={selector}")
                continue
        logger.debug("옵션 <a>에서 group_id 추출 실패")
        return None
    except Exception as e:
        logger.error(f"전체 함수 오류: {e}")
        return None
